...
Write-Host "Compress: $($PublishParameters['CompressPackage'])"
Write-Host "Package path: $ApplicationPackagePath"

Compress-Archive -Path $ApplicationPackagePath\* -DestinationPath $ApplicationPackagePath\..\Package.zip

Remove-Item $ApplicationPackagePath\..\Package.sfpkg
Rename-Item -Path $ApplicationPackagePath\..\Package.zip -NewName Package.sfpkg -Force

$ctx = New-AzureStorageContext -StorageAccountName "Account for fix" -StorageAccountKey "Key for Storage Account"

Remove-AzureStorageBlob -Blob "Pryaniky3.sfpkg" -Container 'tmp' -Context $ctx -Force
Set-AzureStorageBlobContent -File $ApplicationPackagePath\..\Package.sfpkg -Container 'tmp'  -Blob "Package.sfpkg"  -Context $ctx -Force
...
else
{
    ...
    Write-Host "@PublishParameters"+@PublishParameters
    Publish-NewServiceFabricApplication2 @PublishParameters
}